...
Predicate抽出や関数分割などで新しい内部関数を作成する前に、[Local Function Naming](references/QUALITY_RULES.md#local-function-naming) に従い、同一ファイル・同一モジュールの既存語彙と命名規約を確認する。
...
- 新規内部関数の名前が既存の規約・語彙と整合し、責務を表している
- 新規警告が増えていない
